...
# Append the following code below the existing PoseDetection code
...
while i <= n_ppl:
    ...
    Neutral_angle = Angle(A, B, C);

    D = ((A[0]+B[0])/2, (A[1]+B[1])/2) # Centre of hip

    Neutral_length = math.sqrt(lengthSquare(B, D))

    sheet.cell(row=i + 1, column=9).value = X_left
    sheet.cell(row=i + 1, column=10).value = X_right
    sheet.cell(row=i + 1, column=8).value = Neutral_angle
    sheet.cell(row=i + 1, column=7).value = Neutral_length
